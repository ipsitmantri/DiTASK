...
def get_head(task, backbone_channels, num_outputs, config=None, multiscale=True):
    ...
    if head_type == "hrnet":
        ...
        from models.seg_hrnet import HighResolutionHead, HighResolutionHeadCLIP

        if config.MODEL.TYPE == "swin":
            return HighResolutionHead(backbone_channels, num_outputs)
        elif config.MODEL.TYPE == "vit":
            return HighResolutionHeadCLIP(backbone_channels, num_outputs)
        else:
...
class MultiTaskSwin(nn.Module):
    def __init__(self, encoder, config):
        ...
        self.DITASK = config.MODEL.DITASK
        if self.DITASK.ENABLED:
